...
function openEntry(id) {
	...
	$("#" + id + ".entry-wrapper").css('display','block');
	selectEntry(id);
...
function expandEntryTitle(id) {
	...
	*/
	$("#" + id + ".entry-title").promise().done(function(){
		$(this).closest('div.log-entries').addClass('log-entries-active');
		$(this).children('.entry-title-inner').addClass('entry-title-active');
	});
	$('#' + id + '.entry-wrapper.selected').css('cursor','default');
	// var entryText = $("#" + id + ".entry-content.selected p").text();
	$("#play-icon")
		.css('display','block')
		.on('click', function(){
			speak($("#" + id + ".entry-content.selected p").first().text());
	});
}
...
function closeEntry(id) {
	...
	$(".entry-wrapper").promise().done(function() {
		$("#" + id + ".entry-wrapper").find(".edit").animate({opacity: 0}, function() {$(this).css({display: "none"})})
		closeMoveEntryWrapper(id);
	});
	$("#" + id + ".entry-title").closest('div.log-entries').removeClass('log-entries-active');
	$("#" + id + ".entry-title .entry-title-inner").removeClass('entry-title-active');
	$('#' + id + '.entry-wrapper').css('cursor','pointer');
	$('#create-entry').removeClass('create-entry-active');
}
...
// topbar-options mouseenter
$(".topbar-option").mouseenter(function(event) {
	$(this).prev('.topbar-option-inner').css('color', 'cyan');
	$(this).css('background', '#05353A url("../res/holo.png")');
	/*
	$(this).animate({
		"box-shadow": "0px 0px 1px black",
		"background": "#05353A url('../res/holo.png')"
	}, hoverAnimationDuration);
	*/
}).mouseleave(function(event) {
	$(this).prev('.topbar-option-inner').css('color', '#333');
	$(this).css('background', 'rgba(99, 99, 99, 0.2)');
	/*
	$(this).animate({
	...
	*/
}); // topbar-options mouseenter end

$('.topbar-option-inner').mouseenter(function(){
	$(this).next(".topbar-option").css('background', '#05353A url("../res/holo.png")');
}).mouseleave(function(){
	$(this).next(".topbar-option").mouseleave(function(){
		$(this).css('background', 'rgba(99, 99, 99, 0.2)');
	});
});
...
function appendUserLogs(username, hashlist) {
	$.get('/control/getuser?username='+username, function(data) {
		...
		if (createEntryVisible) {
			$("#create-entry").css({display:"block"});
		} else {
			$("#create-entry").css({display:"none"});
		}
		// $(".log-entries").html(s)
		// add entries
		for (i = 0; i < user.entries.length; i++) {
			var e = user.entries.length - i - 1;
			var $div = $("<div>", {id: "entry-" + e, name: "user/" + username + "/entry-" + e, class: "entry entry-wrapper", style:"top:" + 6 *(i+createEntryVisible)+"px;display:block"});
			var $title = $("<div>", {id: "entry-" + e, name: "user/" + username + "/entry-" + e, class: "entry entry-title", html: "<span class='entry-title-inner'>" + user.entries[e].title + '</span>'});
...
function hashChanged() {
	...
	if (/^createentry$/.test(hashlist[0])) {
		...
		$( ".entry" ).promise().done(function() {
			...
			$('#create-entry.selected').promise().done(function(){
				$('#create-entry').addClass('create-entry-active');
			});
		});
		return;
	}
...
// sound fx
$(".topbar-wrapper li, .topbar-option-inner, a, input[type=submit], #create-entry").each(function(){
	$(this).on('click',function(){
		$('#screenShiftSound')[0].play();
	});
});
$(".topbar-wrapper li") // loop each menu item
  .each(function(i) {
    if (i != 0) { // only clone if more than one needed
      $("#menuHoverSound")
        .clone()
        .attr("id", "menuHoverSound-" + i)
        .appendTo($(this).parent()); 
    }
    $(this).data("beeper", i); // save reference 
  })
  .mouseenter(function() {
    $("#menuHoverSound-" + $(this).data("beeper"))[0].play();
  });
$("#menuHoverSound").attr("id", "menuHoverSound-0"); // get first one into naming convention
// sound fx end
